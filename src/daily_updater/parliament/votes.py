...
def get_party_approvals(data_initiatives_votes: pd.DataFrame) -> pd.DataFrame:
    ...
    new_values = data_initiatives_votes.loc[unanime_rows, parties_vote_direction_fields].applymap(
        lambda x: "afavor" if pd.isna(x) else x)  # move all NaN values to "afavor"
    ...
    return data_initiatives_votes.groupby("iniciativa_autor").apply(
        lambda x: calculate_vote_distribution(x, parties_vote_direction_fields)).sort_values("total_iniciativas",
                                                                                             ascending=False)
...
def collect_parties_strange_votes(data_initiatives_votes: pd.DataFrame) -> pd.DataFrame:
    ...
    * In Portugal the deputies must vote equal to the party, but in some situations
    ...
    for col in parties_columns:
        party = col.replace("iniciativa_votacao_", "").replace("cr", "CRISTINA RODRIGUES").replace("jkm",
                                                                                                   "JOACINE KATAR MOREIRA").upper()

        mask = (data_initiatives_votes["iniciativa_autor"] == party) & (
                    (data_initiatives_votes[col] != "afavor") | data_initiatives_votes[col].isnull())
...
def get_initiatives(data_initiatives_votes: pd.DataFrame) -> pd.DataFrame:
    ...
    data_initiatives_votes[
        "iniciativa_url_res"] = "https://www.parlamento.pt/ActividadeParlamentar/Paginas/DetalheIniciativa.aspx?BID=" + \
                                data_initiatives_votes["iniciativa_id"]

    columns = ["iniciativa_evento_fase", "iniciativa_titulo", "iniciativa_url", "iniciativa_url_res",
               "iniciativa_autor", "iniciativa_autor_deputados_nomes", "iniciativa_evento_data", "iniciativa_tipo",
               "iniciativa_votacao_res"] + parties_vote_direction_fields
