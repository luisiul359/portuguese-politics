...
def get_raw_data_from_blob(blob_container: BlobContainerClient, legislature_name: str) -> List[Dict]:
    ...
    try:
        ...
        return raw_data.get("ArrayOfPt_gov_ar_objectos_iniciativas_DetalhePesquisaIniciativasOut", {}).get(
...
def get_raw_data(path: str) -> List[Dict]:
    ...
    try:
        ...
        return payload.json().get("ArrayOfPt_gov_ar_objectos_iniciativas_DetalhePesquisaIniciativasOut", {}).get(
            "pt_gov_ar_objectos_iniciativas_DetalhePesquisaIniciativasOut", [])
    except Exception as e:
        print(path)
        raise e
...
def get_initiatives(raw_initiatives: List) -> pd.DataFrame:
    ...
    for initiative in tqdm(raw_initiatives, "getting_initiatives", file=sys.stdout):
        ...
        info_to_store["iniciativa_autor_grupos_parlamentares"] = "|".join([mydict(autor).get("GP", "") for autor in
                                                                           to_list(initiative.get(
                                                                               "iniAutorGruposParlamentares", {}).get(
        ...
        info_to_store["iniciativa_autor_outros_autor_comissao"] = initiative.get("iniAutorOutros", {}).get(
            "iniAutorComissao", "")

        # iniAutorDeputados
        autor_deputados = to_list(
            initiative.get("iniAutorDeputados", {}).get("pt_gov_ar_objectos_iniciativas_AutoresDeputadosOut", []))
        ...
        # iniciativasOrigem
        origem = to_list(
            mydict(initiative.get("iniciativasOrigem", {})).get("pt_gov_ar_objectos_iniciativas_DadosGeraisOut", []))
        ...
        for event in events:
            ...
            info_to_store_details["iniciativa_publicacao_Tipo"] = [mydict(x).get("pubTipo", "") for x in
                                                                   publicacao_detalhe]
            info_to_store_details["iniciativa_publicacao_URL"] = [mydict(x).get("URLDiario", "") for x in
                                                                  publicacao_detalhe]
            ...
            iniciativas_conjuntas = to_list(
                event.get("iniciativasConjuntas", {}).get("pt_gov_ar_objectos_iniciativas_DiscussaoConjuntaOut", []))
            ...
            info_to_store_details["iniciativa_oradores_deputados_nomes"] = "|".join([
                "|".join([
                    ...
                    for orador in
                    to_list(intervencao.get("oradores", {}).get("pt_gov_ar_objectos_peticoes_OradoresOut", []))
                ])
            ...
            info_to_store_details["iniciativa_oradores_videos"] = "|".join([
                "|".join([
                    "|".join([x.get("link", "") for x in to_list(
                        mydict(orador).get("linkVideo", {}).get("pt_gov_ar_objectos_peticoes_LinksVideos", {}))])
                    for orador in
                    to_list(mydict(intervencao).get("oradores", {}).get("pt_gov_ar_objectos_peticoes_OradoresOut", []))
                ])
                for intervencao in intervencoes
            ])
            ...
            info_to_store_details["iniciativa_comissao_pedidos_parecer"] = "|".join(
                [_ for _ in to_list(comissao.get("pedidosParecer", {}).get("string", [])) if isinstance(_, str)])
            info_to_store_details["iniciativa_comissao_pareceres_recebidos"] = "|".join(
                [_ for _ in to_list(comissao.get("pareceresRecebidos", {}).get("string", [])) if isinstance(_, str)])
            ...
            info_to_store_details["iniciativa_comissao_publicacao_Tipo"] = [mydict(x).get("pubTipo", "") for x in
                                                                            publicacaoDetalhe]
            info_to_store_details["iniciativa_comissao_publicacao_URL"] = [mydict(x).get("URLDiario", "") for x in
                                                                           publicacaoDetalhe]
            info_to_store_details["iniciativa_comissao_publicacao_Obs"] = [mydict(x).get("obs", "") for x in
                                                                           publicacaoDetalhe]
...
def _split_vote_result(vote: str) -> Dict[str, list]:
    ...
    vote = vote.replace("afavor:", ",afavor,").replace("contra:", ",contra,").replace("ausência:",
                                                                                      ",ausência,").replace(
        "abstenção:", ",abstenção,")
...
def get_initiatives_votes(initiatives: pd.DataFrame) -> pd.DataFrame:
    ...
    columns_to_keep = ["iniciativa_id", "iniciativa_nr", "iniciativa_tipo", "iniciativa_titulo",
                       "iniciativa_evento_fase", "iniciativa_evento_data", "iniciativa_url", "iniciativa_obs",
                       "iniciativa_texto_subst", "iniciativa_autor_grupos_parlamentares",
                       "iniciativa_autor_outros_nome", "iniciativa_autor_outros_autor_comissao",
                       "iniciativa_autor_deputados_nomes", "iniciativa_autor_deputados_GPs", "iniciativa_votacao_res",
                       "iniciativa_votacao_desc", "iniciativa_autor", "iniciativa_votacao_unanime"]
